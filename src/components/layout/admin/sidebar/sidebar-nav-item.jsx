  ...
  return (
    <div className="mb-1">
      {!sidebarOpen ? (
        ...
        hasChildren ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className={collapsedBtnClass} style={{ cursor: 'pointer' }}>
                {activeBar}
                <IconComponent
                  className={cn("h-5 w-5 shrink-0 transition-colors", isActive ? "text-primary-foreground" : "text-black")}
                  style={{ strokeWidth: 3 }}
                />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="right" sideOffset={10} className="w-56">
              ...
              {item.items.map((subItem) => {
                ...
                return (
                  ...
                  >
                    <Link href={subItem.url} onClick={onClick} className="flex items-center px-2 py-1.5 w-full">
                      <SubItemIcon className="mr-2 h-5 w-5" style={{ strokeWidth: 2 }} />
          ...
          // Version simple avec tooltip — Link pour le prefetching
          <Tooltip delayDuration={300}>
            <TooltipTrigger asChild>
              <Button variant="ghost" asChild className={collapsedBtnClass} style={{ cursor: 'pointer' }}>
        ...
        // Version étendue
        hasChildren ? (
          // Section avec sous-items : toggle expand
          <Button
            variant="ghost"
            onClick={() => toggleSection(item.title)}
            className={expandedBtnClass}
            style={{ cursor: 'pointer' }}
          >
            {activeBar}
            <IconComponent
              className={cn("h-5 w-5 shrink-0 transition-colors ml-1", isActive ? "text-primary-foreground" : "text-black")}
              style={{ strokeWidth: 3 }}
            />
            <span className={cn("ml-2 text-sm transition-colors", isActive ? "text-primary-foreground font-semibold" : "")}>
          ...
          // Item simple : Link pour le prefetching
          <Button variant="ghost" asChild className={expandedBtnClass} style={{ cursor: 'pointer' }}>
            <Link href={item.url} onClick={onClick}>
              {activeBar}
              <IconComponent
                className={cn("h-5 w-5 shrink-0 transition-colors ml-1", isActive ? "text-primary-foreground" : "text-black")}
            ...
            return (
              ...
              >
                <Link href={subItem.url} onClick={onClick}>
                  ...
                  <SubIcon
                    className={cn("h-5 w-5 mr-2 transition-colors", isSubActive ? "text-primary-foreground" : "text-black")}
                    style={{ strokeWidth: 2 }}
                  />
                  <span className={cn("transition-colors", isSubActive ? "font-semibold" : "")}>
