...
import { ChevronDown, Menu, X, Globe, Phone, Mail, MapPin, ArrowRight, Clock } from 'lucide-react';
...
export function Header() {
  ...
  const navLinks = [
    ...
    { 
      ...
      href: '/services',
      dropdown: [
        { name: 'Transport Maritime', href: '/services/maritime', icon: '🚢', desc: 'Fret maritime international', color: 'bg-blue-50 text-blue-600' },
        { name: 'Transport Aérien', href: '/services/aerien', icon: '✈️', desc: 'Livraison express mondiale', color: 'bg-sky-50 text-sky-600' },
        { name: 'Transport Routier', href: '/services/routier', icon: '🚛', desc: 'Distribution terrestre', color: 'bg-green-50 text-green-600' },
        { name: 'Dédouanement', href: '/services/dedouanement', icon: '📋', desc: 'Procédures douanières', color: 'bg-orange-50 text-orange-600' },
        { name: 'Entreposage', href: '/services/entreposage', icon: '🏪', desc: 'Stockage sécurisé', color: 'bg-purple-50 text-purple-600' },
        { name: 'Assurance Transport', href: '/services/assurance', icon: '🛡️', desc: 'Protection complète', color: 'bg-red-50 text-red-600' }
      ]
    },
  ...
  return (
    <>
      ...
      <div className={`bg-gradient-to-r from-blue-900 to-[#010066] text-white transition-all duration-300 ${
        scrolled ? 'py-1' : 'py-2'
      } hidden lg:block`}>
      ...
      <nav className={`fixed w-full z-50 transition-all duration-500 transform ${
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3 group cursor-pointer">
              ...
              <div className="hidden sm:block">
                ...
                <div className="text-xs text-gray-500 -mt-1">Transport & Logistique</div>
              </div>
            </div>
                      ...
                      <div className="p-3">
                        <div className="grid gap-1">
                          {link.dropdown.map((item, index) => (
                            ...
                            >
                              <div className={`w-10 h-10 rounded-lg ${item.color} flex items-center justify-center group-hover/item:scale-110 transition-transform duration-200`}>
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <div className="relative w-6 h-6">
                  <Menu className={`w-6 h-6 text-gray-700 absolute transition-all duration-300 ${isOpen ? 'opacity-0 rotate-90' : 'opacity-100 rotate-0'}`} />
                  <X className={`w-6 h-6 text-gray-700 absolute transition-all duration-300 ${isOpen ? 'opacity-100 rotate-0' : 'opacity-0 -rotate-90'}`} />
                </div>
              </button>
            </div>
          </div>
                    ...
                    <div className="ml-4 space-y-1">
                      {link.dropdown.map((item) => (
                        <button
                          ...
                          onClick={() => setIsOpen(false)}
                        >
                          <div className={`w-8 h-8 rounded-lg ${item.color} flex items-center justify-center group-hover/mobile:scale-110 transition-transform duration-200`}>
