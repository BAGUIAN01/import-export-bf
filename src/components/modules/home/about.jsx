...
import { Heart, Shield, Clock, Users, Package, ArrowRight, CheckCircle } from 'lucide-react';
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`text-center mb-12 lg:mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-full px-4 lg:px-6 py-2 lg:py-3 mb-6 shadow-sm">
            <Heart className="w-4 h-4 lg:w-5 lg:h-5" style={{ color: '#010066' }} />
            <span className="font-semibold text-sm lg:text-base" style={{ color: '#010066' }}>À Propos</span>
          </div>
          ...
          <p className="text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Depuis 8 ans, nous facilitons les liens familiaux en transportant vos colis 
            avec soin, sécurité et ponctualité.
          </p>
        </div>

        {/* Main Content */}
        <div className={`grid lg:grid-cols-2 gap-12 lg:gap-16 items-center mb-16 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
          
          {/* Left - Story */}
          <div>
            <h3 className="text-2xl lg:text-3xl font-black mb-6" style={{ color: '#010066' }}>
              Notre Mission
            </h3>
            
            <div className="space-y-6">
              <p className="text-lg text-gray-700 leading-relaxed">
                Nous avons créé un service <strong>simple, fiable et abordable</strong> pour 
                répondre aux besoins des familles burkinabées installées en France.
              </p>
              ...
              <div className="bg-gradient-to-r from-orange-50 to-orange-100 rounded-2xl p-6 border border-orange-200">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-orange-800 mb-2">Chaque colis compte</h4>
                    <p className="text-orange-700">
                      Nous transportons bien plus que des objets : nous acheminons l'amour, 
                      le soutien et les rêves de familles séparées par la distance.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {highlights.map((item, index) => (
                  <div key={index} className="text-center bg-white rounded-xl p-4 shadow-md border border-gray-100">
                    <item.icon className="w-6 h-6 mx-auto mb-2" style={{ color: '#010066' }} />
                    <div className="text-xl font-black" style={{ color: '#010066' }}>{item.number}</div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {values.map((value, index) => (
                ...
                >
                  <div className={`bg-white rounded-2xl p-5 shadow-lg border-2 transition-all duration-300 h-full ${
                    activeValue === index 
                      ? 'border-orange-200 shadow-xl' 
                      : 'border-gray-100 hover:border-gray-200'
                  }`}>
                    <div className={`w-10 h-10 bg-gradient-to-br ${value.color} rounded-xl flex items-center justify-center mb-3`}>
                      <value.icon className="w-5 h-5 text-white" />
                    </div>
                    
                    <h4 className="text-base font-bold mb-2" style={{ color: '#010066' }}>
                      {value.title}
                    </h4>
